...
def ask_ai(question, market_data):
    market_summary = []
    for _, row in market_data.iterrows():
        market_summary.append(
            ...
            f"Market Cap=${row['market_cap']:,.0f}, "
            f"Volume=${row['total_volume']:,.0f}, "
            f"Volatility Score={row['volatility_score']:,.0f}"
        )

    prompt = f"""You are a professional crypto market analyst AI agent with deep knowledge of blockchain, trading, and market analysis.
...
3. Specific buy/sell suggestions with reasoning
4. Risk warnings if any

Keep your response practical, data-driven, and easy to understand.
Format your response with clear sections using emojis for better readability."""
...
st.markdown('<div class="section-title">🤖 AI Crypto Agent</div>', unsafe_allow_html=True)
...
with col_ai1:
    user_question = st.text_input(
        label="Ask the AI",
        placeholder="e.g. Which coin should I buy today? What is Bitcoin doing? Is the market bullish?",
        label_visibility="collapsed"
    )

with col_ai2:
    ask_button = st.button("🤖 Ask AI Agent", use_container_width=True)

quick_col1, quick_col2, quick_col3, quick_col4 = st.columns(4)
with quick_col1:
    if st.button("📈 Best coin to buy?", use_container_width=True):
        user_question = "Which coin is the best to buy right now based on current data?"
        ask_button = True
with quick_col2:
    if st.button("⚠️ Market risks?", use_container_width=True):
        user_question = "What are the biggest risks in the market right now?"
        ask_button = True
with quick_col3:
    if st.button("🔥 Trending coins?", use_container_width=True):
        user_question = "Which coins are trending and gaining momentum?"
        ask_button = True
with quick_col4:
    if st.button("📊 Market summary?", use_container_width=True):
        user_question = "Give me a complete market summary and outlook."
        ask_button = True

if ask_button and user_question:
    with st.spinner("🤖 AI Agent is analyzing the market..."):
        ai_response = ask_ai(user_question, df)
    st.markdown(f'<div class="ai-response">{ai_response}</div>', unsafe_allow_html=True)
elif ask_button:
    st.warning("Please type a question or click a quick button above.")
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown('<p style="text-align:center; color:#1e3a5f; font-size:0.75rem; letter-spacing:2px;">CRYPTOSPHERE ANALYTICS • POWERED BY COINGECKO, SUPABASE & GEMINI AI</p>', unsafe_allow_html=True)
